...
function CreateOrderModal({ open, onClose }: CreateOrderModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t("store:modals.createOrder.successToast"));
      handleClose();
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : t("store:modals.createOrder.errorToast"),
      );
    }
  };

  const statusOptions: { value: OrderStatus; labelKey: string }[] = [
    { value: "pending", labelKey: "store:modals.createOrder.statusPending" },
    {
      value: "confirmed",
      labelKey: "store:modals.createOrder.statusConfirmed",
    },
    { value: "shipped", labelKey: "store:modals.createOrder.statusShipped" },
    {
      value: "completed",
      labelKey: "store:modals.createOrder.statusCompleted",
    },
    {
      value: "cancelled",
      labelKey: "store:modals.createOrder.statusCancelled",
    },
  ];

  const paymentOptions: { value: PaymentMethod; labelKey: string }[] = [
    { value: "card", labelKey: "store:modals.createOrder.paymentCard" },
    { value: "usd", labelKey: "store:modals.createOrder.paymentUsd" },
    { value: "euro", labelKey: "store:modals.createOrder.paymentEuro" },
    { value: "cup", labelKey: "store:modals.createOrder.paymentCup" },
  ];
      ...
      >
            ...
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbItem className="text-foreground font-medium text-sm">
                  {t("store:modals.createOrder.title")}
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </DialogTitle>
          <DialogDescription className="sr-only">
            {t("store:modals.createOrder.description")}
          </DialogDescription>
        </DialogHeader>
        ...
        >
          <div className="flex-1 min-h-0 overflow-y-auto space-y-6 px-6">
            <div>
              <Input
                ...
                placeholder={t(
                  "store:modals.createOrder.shippingAddressPlaceholder",
                )}
                autoFocus
                required
              ...
              <div className="flex gap-3 mt-3">
                <Input
                  ...
                  placeholder={t("store:modals.createOrder.cityPlaceholder")}
                  required
                  className="flex-1"
                />
                <Input
                  ...
                  placeholder={t(
                    "store:modals.createOrder.provincePlaceholder",
                  )}
                  required
                  className="flex-1"
                />
                <Input
                  ...
                  placeholder={t("store:modals.createOrder.phonePlaceholder")}
                  className="flex-1"
                />
              </div>
            </div>
                  ...
                  {selectedCustomerId ? (
                    <span className="flex-1 truncate">
                      {customers.find((c) => c.id === selectedCustomerId)
                        ?.name ?? t("store:modals.createOrder.unknownCustomer")}
                    </span>
                  ) : (
                    <SelectValue
                      placeholder={t("store:modals.createOrder.selectCustomer")}
                    />
                  )}
                </SelectTrigger>
                <SelectContent>
                  ...
                  {customers.length === 0 && (
                    <SelectItem value="__none" disabled>
                      {t("store:modals.createOrder.noCustomers")}
                    </SelectItem>
                  )}
                </SelectContent>
              </Select>
                    ...
                    <span>
                      {statusOptions.find((s) => s.value === orderStatus)
                        ? t(
                            statusOptions.find((s) => s.value === orderStatus)
                              ?.labelKey ??
                              "store:modals.createOrder.statusLabel",
                          )
                        : t("store:modals.createOrder.statusLabel")}
                    </span>
                  </button>
                </PopoverTrigger>
                      ...
                      {paymentOptions.find((p) => p.value === paymentMethod)
                        ? t(
                            paymentOptions.find(
                              ...
                              "store:modals.createOrder.paymentLabel",
                          )
                        : t("store:modals.createOrder.paymentLabel")}
                    </span>
                  </button>
                </PopoverTrigger>
                <PopoverContent className="w-48 p-1" align="start">
            ...
            <Textarea
              ...
              placeholder={t("store:modals.createOrder.notesPlaceholder")}
              rows={2}
            />
          </div>
          ...
          <DialogFooter className="shrink-0 border-t border-border bg-background px-6 py-4">
            ...
            >
              {isPending
                ? t("store:modals.createOrder.creating")
                : t("store:modals.createOrder.createButton")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
