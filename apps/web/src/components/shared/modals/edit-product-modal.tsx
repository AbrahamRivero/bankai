...
import useUpdateProduct from "@/hooks/mutations/store/use-update-product";
import useGetProduct from "@/hooks/queries/store/use-get-product";
...
function EditProductModal({ open, onClose, productId }: EditProductModalProps) {
  ...
  const { canUpdateProducts } = useWorkspacePermission();
  ...
  const uploadImage = async (file: File) => {
    ...
    if (!response.ok) {
      throw new Error(t("store:modals.editProduct.imagesUploadError"));
    }
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : t("store:modals.editProduct.imagesUploadError"),
      );
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t("store:modals.editProduct.successToast"));
      handleClose();
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : t("store:modals.editProduct.errorToast"),
      );
    }
  };
      ...
      >
        <DialogHeader className="px-3 pt-4 pb-1 gap-1.5 shrink-0">
          <DialogTitle className="sr-only">
            {t("store:modals.editProduct.title")}
          </DialogTitle>
          <DialogDescription className="sr-only">
            {t("store:modals.editProduct.description")}
          </DialogDescription>
        ...
        >
          <div className="flex-1 min-h-0 overflow-y-auto space-y-6 px-6">
            <div>
              ...
              <Input
                ...
                placeholder={t("store:modals.editProduct.titlePlaceholder")}
                autoFocus
                required
                className="mt-1"
              />
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder={t(
                  "store:modals.editProduct.descriptionPlaceholder",
                )}
                rows={3}
                className="mt-1"
              />
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <Label htmlFor="edit-product-gender">
                ...
                >
                  <SelectTrigger id="edit-product-gender" className="mt-1">
                    <SelectValue
                      placeholder={t(
                        "store:modals.editProduct.genderPlaceholder",
                      )}
                    />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="men">
                      {t("store:modals.editProduct.genderMen")}
                    </SelectItem>
                    <SelectItem value="women">
                      {t("store:modals.editProduct.genderWomen")}
                    </SelectItem>
                    <SelectItem value="kid">
                      {t("store:modals.editProduct.genderKid")}
                    </SelectItem>
                    <SelectItem value="unisex">
                      {t("store:modals.editProduct.genderUnisex")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
            ...
            <div>
              <Label>{t("store:modals.editProduct.sizesLabel")}</Label>
              <div className="mt-1">
                <Combobox
                  multiple
                  value={sizes}
                ...
                >
                  <ComboboxChips>
                    <ComboboxChipsInput
                      placeholder={t(
                        "store:modals.editProduct.sizesPlaceholder",
                      )}
                    />
                  </ComboboxChips>
                  <ComboboxPopup>
            ...
            <div>
              <Label>{t("store:modals.editProduct.tagsLabel")}</Label>
              <div className="flex gap-2 mt-1">
                <Input
                  ...
                  placeholder={t("store:modals.editProduct.tagsPlaceholder")}
                />
                <Button
                  type="button"
                  size="sm"
            ...
            <div>
              <Label>{t("store:modals.editProduct.imagesLabel")}</Label>
                ...
                >
                  {isUploading ? (
                    <span className="text-xs text-muted-foreground">
                      {t("store:modals.editProduct.imagesUploading")}
                    </span>
                  ) : (
            ...
            <div>
              <Label>
                {t("store:modals.editProduct.relatedProductsLabel")}
              </Label>
                    ...
                    {selectedProductId ? (
                      <span className="flex-1 truncate">
                        {products.find((p) => p.id === selectedProductId)
                          ?.title ??
                          t("store:modals.editProduct.unknownProduct")}
                      </span>
                    ) : (
                      <SelectValue
                        placeholder={t(
              ...
              {relatedProductIds.length > 0 ? (
                <div className="space-y-1 mt-2">
                  {relatedProductIds.map((relId) => {
                    const product = products.find((p) => p.id === relId);
                    return (
                      <div
                        key={relId}
                      ...
                      >
                        ...
                        <Button
                          ...
                          onClick={() => handleRemoveProduct(relId)}
                        >
                          <X className="h-3 w-3" />
                        </Button>
                      </div>
          ...
          <DialogFooter className="shrink-0 border-t border-border bg-background px-6 py-4">
            ...
            >
              {isPending
                ? t("store:modals.editProduct.saving")
                : t("store:modals.editProduct.saveButton")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
